...
object BinaryTreeMain {
  def main(args: Array[String]): Unit = {
    import BinaryTree.{BinaryTree, BinaryTreeImpl}

    case class Dinosaur(name: String, age: Int)

    val binaryTree: BinaryTree[Dinosaur] = new BinaryTreeImpl[Dinosaur]()((x: Dinosaur, y: Dinosaur) => {
      if (x.age < y.age) -1
      else if (x.age == y.age) 0
      else 1
    })

    binaryTree.add(Dinosaur(name = "John", age = 22))
    binaryTree.add(Dinosaur(name = "Paul", age = 23))
    binaryTree.add(Dinosaur(name = "Jack", age = 8))
    binaryTree.add(Dinosaur(name = "John", age = 22))

    binaryTree.traverse(println)

    println()

    binaryTree.remove(Dinosaur(name = "Paul", age = 23))

    binaryTree.traverse(println)

    println()

    binaryTree.remove(Dinosaur(name = "John", age = 22))

    binaryTree.traverse(println)

    println()

    binaryTree.remove(Dinosaur(name = "Jack", age = 8))
    ...
    println(binaryTree.find(Dinosaur(name = "Paul", age = 23)))
    println(binaryTree.find(Dinosaur(name = "John", age = 22)))
  }
}
